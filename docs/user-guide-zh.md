...
- **确认**（`⌘+Enter`）— 输入文本并关闭面板；在最终文本编辑区外按 `Enter` 也可确认，编辑区内按 `Enter` 为换行
- **复制到剪贴板**（`⌘+Shift+Enter`）— 将文本复制到剪贴板而非输入
- **取消**（`Esc`）— 丢弃文本
- **编辑** — 点击文本区域可在确认前修改
...
可在设置 → 麦克风中开启“降低系统音量”：录音期间降低 Mac 上全部播放声音，结束后自动恢复，包括浏览器、音乐和会议音频。该功能会按比例降低当前音量并设置安全上限，但无法消除 macOS 或蓝牙重新配置音频路由时产生的短暂中断。
...
|---|---|
| **编辑文本** | 点击文本区域并输入 |
| **复制到剪贴板** | `⌘+Shift+Enter` — 复制而非输入到当前应用 |
...
| `⌘+Enter` | 预览面板 | 确认并输入文本（编辑区内 `Enter` 为换行） |
| `⌘+Shift+Enter` | 预览面板 | 复制到剪贴板 |
| `Esc` | 预览面板/流式悬浮窗 | 取消 |
| `⌘1` – `⌘9` | 预览面板 | 切换增强模式 |
